...
export default function Main() {
  ...
  const handleGenerate = async () => {
    ...
    setTimeout(() => {
      setResult("This is a refined response from the AI!");
  ...
  const handleRegenerate = async () => {
    ...
    setIsGenerating(true);
    setTimeout(() => {
      setResult("This is a regenerated response based on your feedback.");
      setImprovePrompt("");
      setIsImprovingField(false);
      setIsGenerating(false);
    }, 1000);
    ...
    <main className="max-sm:w-full max-sm:px-2">
      <div className="w-[60vw] max-lg:w-[80vw] max-sm:w-full relative pt-6 pb-2 px-4 rounded-xl dark:border-white/20 border-black/40 bg-white bg-opacity-10 backdrop-blur-xl border flex flex-col items-center justify-center dark:shadow-none shadow-none z-50">
        ...
        <div className="flex justify-between items-end w-full mt-6">
          ...
          <div>
            <button
              className="btn bg-[rgba(229,231,235,0.3)] dark:bg-[rgba(0,0,0,0.05)] rounded-lg backdrop-blur-lg -webkit-backdrop-blur-lg border border-gray-400/50 dark:border-white/20 dark:text-white p-2 dark:hover:bg-[rgba(255,255,255,0.05)] hover:bg-[rgba(0,0,0,0.05)]"
              onClick={handleGenerate}
              disabled={isGenerating}
            >
              {isGenerating ? (
                <HiStop className="text-xs animate-pulse" />
              ) : (
